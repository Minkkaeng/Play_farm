...
const Home = ({ farmData }) => {
  ...
  data.forEach((item) => {
    if (
      ...
      if (
        addressList.length &&
        addressList.includes(siguNm(item.RDNMADR) || siguNm(item.REFINE_LOTNO_ADDR) || siguNm(item.REFINE_ROADNM_ADDR))
      )
        return;
    ...
    <div>
      <div className="category-container">
        ...
        <select name="" id="" onChange={(e) => setSigungu(e.target.value)}>
          <option value="">전체</option>
          {uniqueList(
            addressList.map((v) => (district ? v.split(" ")[0] === district && v.split(" ")[1] : v.split(" ")[1]))
          )
            .filter((v) => v)
            .map((address, i) => {
              return (
        ...
        <select name="experience" id="experience" onChange={(e) => setExperience(e.target.value)}>
          ...
          {uniqueList(experienceList).map((experience, i) => {
            return (
              <option key={i + 1} value={experience}>
                {experience}
              </option>
            );
          })}
        ...
        {allData[currentPage - 1]?.map((item, i) => (
          <li key={i}>
            <div className="imgBox">
              <img
                src={item.VILLAGE_PHOTO ? item.VILLAGE_PHOTO : `${process.env.PUBLIC_URL || ""}/images/temp.png`}
                alt="village"
              />
            </div>
            <div className="infoBox">
              {Object.keys(item)
                .filter((key) => key !== "VILLAGE_PHOTO")
                .map((key) => (
                  <p key={key}>
